...
module.exports = {
    ...
    options: [
        {
            name: 'song',
            description:('A merda que tu quer pesquisar'),
    ...
    async execute({ client, inter }) {
        ...
        if (!res?.tracks.length) return inter.editReply({ content: await Translate(`Isto non ecsiste <❌>`) });
        ...
        collector.on('collect', async (query) => {
            ...
            try {
                ...
                return inter.followUp({ content: await Translate(`Eu não tenho cacife pra entrar nesse morro... <❌>`), ephemeral: true });
            }
